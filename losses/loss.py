...
class Contrast(torch.nn.Module):
    def __init__(self, args, batch_size, temperature=0.1):
        super().__init__()
...
class Loss(torch.nn.Module):
    def __init__(self, batch_size, args,temperature):
        super().__init__()
        self.rot_loss = torch.nn.CrossEntropyLoss().cuda()
        self.recon_loss = torch.nn.L1Loss().cuda()
        self.contrast_loss = Contrast(args, batch_size,temperature).cuda()
        self.alpha1 = 0.0
        self.alpha2 = 1.0
        self.alpha3 = 1.0
